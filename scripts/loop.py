...
class DriverConfig:
    ...
    # REDISPATCH. ``required_check`` defaults to the close-out CI context.
    chat_room: str = ""
    leaf_agent_id: str = ""
...
def _supervise_parser(sub) -> None:
    ...
    p.add_argument(
        "--required-check", default=os.environ.get("LOOP_REQUIRED_CHECK", "validate-skills"),
        help="CI context the supervisor watches (default: validate-skills)",
    )
    p.add_argument("--repo", default=os.environ.get("LOOP_REPO", "CaicoLeung/skills"))
    p.add_argument("--base", default=os.environ.get("LOOP_BASE_BRANCH", "main"))
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    known = {"route", "closeout", "supervise"}
    if argv and argv[0] not in known and not argv[0].startswith("-"):
        argv = ["route", *argv]  # legacy: bare issue number → route

    p = argparse.ArgumentParser(
        description=(
            "Deterministic loop driver (ADR-0008 + ADR-0006): "
            "routing (T5a) + close-out (T5b) + supervisor"
        )
    )
