...
def main() -> int:
    ...
    # =========================================================================
    # AC2b: no CI gate (required_check="") — derived verdict gates (ADR-0013)
    # =========================================================================
    print("no CI gate (empty required_check): gate satisfied without a check:")

    # No required check configured: a merged PR is COMPLETE even though no
    # check ever reported. The driver's derived verdict is the gate, so there
    # is nothing to wait on.
    merged_no_gate = _gate(pr_state="MERGED", required_check="", check_state="",
                           check_seen=False, merged_at="2026-07-26T10:00:00Z")
    _check(merged_no_gate.gated, "empty required_check -> gated is True", failed)
    _check(not merged_no_gate.signal_deadline_exceeded,
           "empty required_check -> absence-of-signal never fires", failed)
    d = supervise.plan_supervise(merged_no_gate, retries_used=0)
    _check(d.action == supervise.ACTION_COMPLETE,
           "merged + no CI gate -> COMPLETE (derived verdict gates)", failed)

    # PR still OPEN, no check seen, long past the deadline: absence-of-signal
    # must NOT escalate — there is no required check to be absent.
    open_no_gate = _gate(pr_state="OPEN", merge_state_status="CLEAN",
                         required_check="", check_state="", check_seen=False,
                         signal_elapsed_sec=10_000)
    d = supervise.plan_supervise(open_no_gate, retries_used=0)
    _check(d.deviation != supervise.DEVIATION_CHECK_MISSING,
           "open + no CI gate -> no check_missing deviation", failed)
