    ...
    if verdict.status == VERDICT_PASS:
        return CloseoutDecision(
            ...
            reason=(
                ...
                "(GitHub merges when validate-skills + review-verdict are green)"
            ),
        )
...
def resolution_comment(
    ...
    return (
        f"Resolved #{issue_number} via the loop-engineering close-out "
        f"(ADR-0008). The derived verdict passed after {rounds} review "
        f"round(s); the loop enabled auto-merge and GitHub merged {pr_url} "
        f"when `validate-skills` and `review-verdict` were green.{sha_line}\n\n"
...
def auto_merge_command(repo: str, pr_number: int) -> list[str]:
    ...
    required checks (``validate-skills`` + ``review-verdict``) are green — so
