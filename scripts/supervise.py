...
class GateState:
    ...
    Attributes:
        ...
        required_check: The CI context the gate watches (e.g.
            ``validate-skills``). From the workflow's SUPERVISE primitive.
    ...
    @property
    def gated(self) -> bool:
        """True iff the required check has genuinely passed (success only)."""
        return self.check_state == "success"
    ...
    def signal_deadline_exceeded(self) -> bool:
        ...
        """
        return (not self.check_seen) and self.signal_elapsed_sec >= self.signal_deadline_sec
