#!/usr/bin/env python3
"""Verdict-selection library + optional consumer-side CI check (T3 / ADR-0007).
The verdict-selection library the loop driver's close-out composes, plus an
optional consumer-side CI entry point. ``select_current_findings`` picks the
reviewer identity's **latest** findings comment whose reviewed-SHA equals the
PR head and feeds it (with the PR's changed files) to ``derive_verdict``
(``scripts/verdict.py``). Nothing self-declares a verdict and nothing appends
a token. ``main()`` wraps the same logic as a GitHub Actions check a consumer
can require — **unused in this repo** (ADR-0013 retired the ``review-verdict``
workflow; the gate is the driver's local derivation), kept as the reusable
gate for consumers who opt into independent CI enforcement.
