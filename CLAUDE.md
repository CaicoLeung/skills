...
Routing dispatches on ticket type (`research` / `prototype` / `grilling` / `task`) after the readiness gate — orthogonal to triage state. Pure dispatcher: `python3 skills/loop-engineering/scripts/routing.py`; tests: `python3 skills/loop-engineering/scripts/test_routing.py`. See `docs/agents/ticket-types.md`.
