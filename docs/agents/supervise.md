...
> Required check `<required_check>` has not reported within the 300s deadline;
> confirm the workflow that emits that context still exists.

This deviation only fires when a `required_check` is configured. ADR-0013 made
the CI gate optional (default empty): with no required check there is nothing
to be absent, so the supervisor neither waits on nor escalates from a missing
check — the gate is the driver's derived verdict alone.
